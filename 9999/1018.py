...
# 이 부분이 조금 아쉬운데 White 하나만 계산해서 Black 은 64 - whiteCase 로 구할 수 있음 둘은 완전히 반대의 경우이기 때문

for _ in range(n):
    line = input()
    memo.append(list(line))
...
for i in range(n - 8 + 1):
    for j in range(m - 8 + 1):
        ...
        for x in range(8):
            for y in range(8):
                if White[x][y] != memo[i + x][j + y]:
                    caseWhite += 1

                if Black[x][y] != memo[i + x][j + y]:
                    caseBlack += 1

        
        whiteOrBlack = min(caseWhite, caseBlack)
        if whiteOrBlack < minChange:
            minChange = whiteOrBlack
